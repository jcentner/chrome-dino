...
- 2026-04-26 slice-4 source half shipped (commit 879e5178). Reward magnitudes lowered: REWARD_STEP 1.0 → 0.1, REWARD_TERMINAL -100.0 → -1.0 (ratio 1:100 → 1:10, sign preserved). ep_rew_mean source switched from `model.logger.name_to_value` (cleared per SB3 dump cycle, returned None for nearly every periodic-eval row in slice 3) to `model.ep_info_buffer` (deque populated by SB3's auto-Monitor wrapper). Per impl §3.3 magnitude tuning is NOT ADR-gated — recorded in slice-4 wrap not as a new ADR (the queued-plan note "new ADR superseding ADR-003 reward subsection" misremembered the structure: ADR-003 is observation-space, no ADR locks reward shape). Reviewer R1 verdict: needs-fixes / 0 Critical / 3 Major — (a) multi-variable change violated "exactly one bounded change", (b)+(c) `exploration_fraction` change is a no-op under chunked `model.learn(reset_num_timesteps=False)` because SB3 resets `_total_timesteps` per chunk, pinning ε at `final_eps=0.05` from chunk 2 onward, and the proposed `learning_starts: 10k` would have transitioned the agent directly from random into broken-ε argmax with no actual ε-greedy exploration phase. Fixes: reverted both hyperparameter changes (learning_starts back to 1_000, exploration_fraction back to 0.1), keeping ONLY reward magnitudes as the slice-4 experimental variable. Logged TD-011 (High) for the broken-ε plumbing — slice-5 candidate; resolution path is to replace the chunked learn loop with a single `model.learn(total_timesteps=total_steps, callback=...)` call where the callback handles checkpoint/eval/CSV cadence, so the linear ε schedule decays over the full budget as documented. TD-008 title fixed (was "diverges to -8" stub from slice-2-era; now "does not return 0.0"). Reviewer R2: pass / 0 Critical / 0 Major. 50 tests pass. Stage → blocked / awaiting-human-decision pending operator launch of 4h training run with the new reward magnitudes (`python scripts/train.py --total-steps 750000 --max-wall-hours 4.0`); slice-4 evidence half (eval trajectory + beat-baseline gate evaluation + slice-3 → slice-4 eval-mean delta + slice-4 wrap) follows in a later session.

- 2026-04-26 slice-4 partial run (informal evidence, NOT canonical): operator launched `dqn-20260426T195443Z` with `--max-wall-hours 2.0` instead of 4.0, run terminated by unrelated issue at ~225k steps / ~37 wall-min. Local-only artifacts (logs/ + models/ are gitignored) retained for cross-checking the canonical 4h run that follows. **Real signal observed** (informs the slice-5 design): (i) reward reshape is doing meaningful work — eval trajectory 54.5 → 48.1 → 62.8 → 56.3 across the 50k/100k/150k/200k checkpoints, eval-200k max=122 (vs slice-3's flat 48-51 range), training `ep_reward_mean` plateaued at ~80 env-steps/episode from 25k onward (~7× slice-3's random-baseline ~110-step episodes). Slice-3's Q-collapse diagnosis was correct; reward reshape unblocked it. (ii) New plateau at training ~80 / eval ~56 with high variance, suggests the policy learned a fixed-threshold "jump when xPos_rel < K" rule that works at slow speeds but cannot adapt to speedup — almost certainly the next bottleneck is the missing closing-time feature (network has to learn `xPos_rel / speed_norm` as a division from raw inputs; ADR-003 §"Δt-invariance" trade-off becoming the binding constraint). (iii) Train-vs-eval pipeline divergence surfaced: `scripts/train.py` runs `DinoEnv` (slice 2) but periodic eval subprocess-invokes `scripts/eval.py` which uses `Browser` directly (slice 1) — same policy, different env-step pacing, makes the train↔eval gap (820 train env-steps vs 56 eval-score-mean) ambiguous. Logged for later as a measurement-tooling concern; not a slice-5 candidate (would compete with the closing-time experiment for attribution). Slice-5 design plan being drafted now while the canonical 4h run executes; queued bounded change is closing-time feature (highest leverage given the plateau shape).

- 2026-04-26 slice-5 plan staged (DRAFT, pre-evidence) at [phases/phase-1-slice-5-plan-DRAFT.md](phases/phase-1-slice-5-plan-DRAFT.md) while the slice-4 canonical 4h training run executes (run dir: logs/train/dqn-20260426T204842Z). Draft includes: Candidate A (closing-time feature, OBS_DIM 14→16, recommended), Candidate B (TD-011 ε-schedule fix, backup), pre-staged ADR-003 amendment block ready to apply on source merge, three open questions for the planner critic (sentinel-for-no-obstacle closing-time value, warm-start vs scratch re-train, speed_norm clamp value). Candidate selection deferred to post-slice-4-wrap based on per-episode score distribution shape: high-variance with speedup-boundary clusters → Candidate A; flat with uniformly-low scores → Candidate B; still climbing past 200k → extend slice-4 training instead. Blocked Reason in state.md updated with the resume routing instructions (run dir, files to read, decision tree, partial-run reference for cross-validation).

- 2026-04-26 resume after slice-4 4h run: canonical run `dqn-20260426T204842Z` completed 750k steps in 2h11m wall (exit_reason=step-budget). Evidence at [docs/wraps/slice-4.md](../docs/wraps/slice-4.md). Eval trajectory `48.95, 69.25, 48.6, 48.1, 48.95, 48.1, 87.0, 58.85, 64.55, 48.1, 48.05, 48.2, 49.8, 48.65, 50.5` — peak at 350k (mean=87, scores 58–108, credible play), final 750k=50.5 (cluster at 48, first-cactus death). Training behavior-policy `ep_reward_mean` rose 74→92 then steady at 82–92, but greedy-eval policy collapsed: train↔eval gap widened from ~0 at 350k to ~+37 at 750k. **Beat-baseline gate (BC-2 both-thresholds): final 50.5 clears absolute floor 48.3 by +2.2 but fails the +50-absolute and ×1.10-relative thresholds; peak 87 also fails them. Gate FAILS at slice-4 final.** Diagnosis flips the slice-5 draft: closing-time feature (Candidate A) targets plateau-at-speedup-boundary at scores 100/200/300, but max observed = 108 — feature capacity is not the binding constraint. Peak-and-collapse with widening greedy↔behavior gap is the textbook signature of broken ε under chunked `model.learn(reset_num_timesteps=False)` (TD-011): behavior policy stays okay at ε=0.05, greedy collapses, replay buffer drift with no exploration to recover. Human selected **Candidate B (TD-011 fix)** for slice 5 and chose `Stage: planning` (re-draft rather than edit the DRAFT). Candidate A deferred to slice 6 or later — representation upgrades should ride on a known-stable training loop. Stage advanced: blocked → planning, Active Slice 4 → 5, slice evidence reset.

## Slice 5 plan (DRAFT, queued, not yet executed)

Pre-staged while the slice-4 canonical 4h training run executes. Full
draft at [phase-1-slice-5-plan-DRAFT.md](phases/phase-1-slice-5-plan-DRAFT.md)
including a pre-staged ADR-003 amendment block ready to apply on slice-5
source merge.

ONE bounded change. Recommended candidate: **closing-time feature** — add
`closing_time_norm = clamp(xPos_rel / max(speed_norm, 0.05), 0, 5)` per
obstacle slot, growing OBS_DIM from 14 → 16. Directly addresses the
slice-4 partial-evidence diagnosis (policy plateaued at training ~80
reward / eval ~56 score with max=122, consistent with a fixed-threshold
"jump when xPos_rel < K" rule that cannot adapt to game-speed ramps
because the network has to learn `xPos_rel / speed_norm` as a division
from raw inputs).

Backup candidate if slice-4 final trajectory shows the plateau is
exploration-driven instead (uniformly-low scores rather than
high-variance with credible max): **fix TD-011** (broken ε schedule
under chunked `model.learn`) — but flagged as a refactor, not a feature,
so doing both at once would violate "one bounded change". Sequence A → B.

Decision point: read the draft after the slice-4 wrap is written. If
slice-4 evidence flips the diagnosis, the draft is replaced rather than
edited.

Reward magnitude analysis remains in /memories/repo/training-observations.md
for reference; closing-time rationale and the queued ADR-003 amendment
text live in the slice-5 draft.
